...
Browser-based VR creation platform for 3D animation and interactive content. SDF sculpting is the primary creation tool. Performance capture animation and URL-based sharing.
...
**Per-hand tool system** (replaces old ModeManager). Each hand independently selects a tool: sculpt (add/subtract/smooth/move), spawn (light), move_layer, inspector, hierarchy. `InteractionManager` routes input to subsystems based on each hand's active tool.
...
- `creation-ux.md` - sculpting UX, tools, composition
- `animation.md` - timeline, performance capture, keyframing
- `rendering.md` - materials, visual style, render pipeline
- `platform.md` - sharing, scripting, multiplayer, AI (future phases)
- `tech-decisions.md` - why browser, why WebGPU, architecture decisions
- `test-harness.md` - automated testing via command bus, XR emulation, visual regression

## Development Phases

1. **MVP**: VR sculpting + basic animation + URL sharing
